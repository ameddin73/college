#include "all.h"
/* context-lists.c 1229c */
/* private functions for updating lists of expressions in contexts 1229e */
static void fill_hole(Exp e, Value v) {
  assert(e->alt == HOLE);
  e->alt = LITERAL;
  e->u.literal = v;
}
/* private functions for updating lists of expressions in contexts 1230a */
static Explist find_explist_hole(Explist es) {
  while (es && es->hd->alt != HOLE)
    es = es->tl;
  return es;
}
/* context-lists.c 1229d */
