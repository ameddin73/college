...
/* adjust the state [[s]] so it's ready to start parsing using row [[t[i]]] 1109a */
    if (t[i].keyword) {
        assert(first != NULL);
        s->input = s->input->tl;
...
/* tableparsing.c 1108f */
static bool rowmatches(struct ParserRow *row, Name first) {
    return row->keyword == NULL || strtoname(row->keyword) == first;
}
/* tableparsing.c 1110a */
Exp parseexp(Par p, Sourceloc source) {
    switch (p->alt) {
    case ATOM:

/* if [[p->u.atom]] is a reserved word, call [[synerror]] with [[source]] 1113a */
