...
Linestream stringlines(const char *stringname, const char *s) {
    ...
    /* check to see that [[s]] is empty or ends in a newline 1077d */
    {   int n = strlen(s);
        assert(n == 0 || s[n-1] == '\n');
    }
    lines->s = s;
...
/* set [[lines->buf]] to next line from file [[lines->fin]], or return [[NULL]] if lines are exhausted 1078c */
        {
...
/* set [[lines->buf]] to next line from string [[lines->s]], or return [[NULL]] if lines are exhausted 1079a */
        {
