...
/* scheme-tests.c 1215e */
TestResult test_result(UnitTest t, Env rho) {
    switch (t->alt) {
    case CHECK_EXPECT:
        /* run [[check-expect]] test [[t]], returning [[TestResult]] 1216a */
        {   if (setjmp(testjmp)) {

/* report that evaluating [[t->u.check_expect.check]] failed with an error 1216d */
                fprint(stderr,
...
/* report that evaluating [[t->u.check_expect.expect]] failed with an error 1216e */
                fprint(stderr,
                     "Check-expect failed: expected %e to evaluate to the same "
            ...
            if (!equalpairs(check, expect)) {
                /* report failure because the values are not equal 1216c */
                fprint(stderr,
    ...
    case CHECK_ERROR:
        /* run [[check-error]] test [[t]], returning [[TestResult]] 1216b */
        {   if (setjmp(testjmp)) {
                return TEST_PASSED; // error occurred, so the test passed
            }
            Value check = eval(t->u.check_error,  rho);

      /* report that evaluating [[t->u.check_error]] produced [[check]] 1217a */
            fprint(stderr,
