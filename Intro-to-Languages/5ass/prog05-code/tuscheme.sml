...
(* exceptions used in languages with type checking 1139b *)
...
(* \footnotesize shared: names, environments, strings, errors, printing, interaction, streams, \&\ initialization 1139a *)
(* for working with curried functions: [[id]], [[fst]], [[snd]], [[pair]], [[curry]], and [[curry3]] 1165c *)
fun id x = x
fun fst (x, y) = x
fun snd (x, y) = y
fun pair x y = (x, y)
...
(* support for names and environments 352 *)
type name = string
(* support for names and environments 353 *)
...
(* support for detecting and signaling errors detected at run time 357d *)
exception RuntimeError of string (* error message *)
(* support for detecting and signaling errors detected at run time 358b *)
...
(* support for detecting and signaling errors detected at run time 358c *)
exception InternalError of string (* bug in the interpreter *)
(* list functions not provided by \sml's initial basis 1143b *)
fun zip3 ([], [], []) = []
...
(* list functions not provided by \sml's initial basis 1143c *)
val reverse = rev
(* list functions not provided by \sml's initial basis 1143d *)
fun optionList [] = SOME []
  ...
  | optionList (SOME x :: rest) =
      (case optionList rest
         of SOME xs => SOME (x :: xs)
          | NONE    => NONE)
(* utility functions for string manipulation and printing 1140a *)
fun println  s = (print s; print "\n")
fun eprint   s = TextIO.output (TextIO.stdErr, s)
fun eprintln s = (eprint s; eprint "\n")
(* utility functions for string manipulation and printing 1140b *)
fun predefinedFunctionError s = 
   if String.isSubstring "left as exercise:" s
      then ()
      else eprintln ("while reading predefined functions, " ^ s)
(* utility functions for string manipulation and printing 1140c *)
fun intString n =
  String.map (fn #"~" => #"-" | c => c) (Int.toString n)
(* utility functions for string manipulation and printing 1140d *)
fun plural what [x] = what
  | plural what _   = what ^ "s"

fun countString xs what =
  intString (length xs) ^ " " ^ plural what xs
(* utility functions for string manipulation and printing 1140e *)
...
(* utility functions for string manipulation and printing 1141a *)
fun printUTF8 code =
...
(* utility functions for string manipulation and printing 1141b *)
...
(* support for representing errors as \ml\ values 1144b *)
datatype 'a error = OK of 'a | ERROR of string
(* support for representing errors as \ml\ values 1145a *)
infix 1 >>=
fun (OK x)      >>= k  =  k x
...
(* support for representing errors as \ml\ values 1145b *)
infix 1 >>=+
fun e >>=+ k'  =  e >>= (OK o k')
(* type declarations for consistency checking *)
val _ = op >>=+ : 'a error * ('a -> 'b) -> 'b error
(* support for representing errors as \ml\ values 1146a *)
fun errorList es =
...
(* type [[interactivity]] plus related functions and value 366a *)
datatype input_interactivity = PROMPTING | NOT_PROMPTING
(* type [[interactivity]] plus related functions and value 366b *)
datatype output_interactivity = PRINTING | NOT_PRINTING
(* type [[interactivity]] plus related functions and value 366c *)
type interactivity = input_interactivity * output_interactivity
val noninteractive = (NOT_PROMPTING, NOT_PRINTING)
...
(* simple implementations of set operations 1142a *)
type 'a set = 'a list
val emptyset = []
fun member x = 
...
(* collections with mapping and combining functions 1142b *)
...
(* collections with mapping and combining functions 1143a *)
...
(* stream transformers and their combinators 1163a *)
type ('a, 'b) xformer = 
  'a stream -> ('b error * 'a stream) option
(* type declarations for consistency checking *)
type ('a, 'b) xformer = ('a, 'b) xformer
(* stream transformers and their combinators 1163b *)
fun pure y = fn xs => SOME (OK y, xs)
(* type declarations for consistency checking *)
val _ = op pure : 'b -> ('a, 'b) xformer
(* stream transformers and their combinators 1165a *)
infix 3 <*>
fun tx_f <*> tx_b =
  fn xs => case tx_f xs
             of NONE => NONE
...
(* stream transformers and their combinators 1165b *)
infixr 4 <$>
fun f <$> p = pure f <*> p
(* type declarations for consistency checking *)
val _ = op <$> : ('b -> 'c) * ('a, 'b) xformer -> ('a, 'c) xformer
(* stream transformers and their combinators 1166a *)
infix 1 <|>
fun t1 <|> t2 = (fn xs => case t1 xs of SOME y => SOME y | NONE => t2 xs) 
(* type declarations for consistency checking *)
val _ = op <|> : ('a, 'b) xformer * ('a, 'b) xformer -> ('a, 'b) xformer
(* stream transformers and their combinators 1166b *)
fun pzero _ = NONE
(* stream transformers and their combinators 1166c *)
fun anyParser ts = 
  foldr op <|> pzero ts
(* type declarations for consistency checking *)
...
(* stream transformers and their combinators 1167c *)
fun eos xs = case streamGet xs
               of NONE => SOME (OK (), EOS)
                | SOME _ => NONE
(* type declarations for consistency checking *)
val _ = op eos : ('a, unit) xformer
(* stream transformers and their combinators 1168a *)
fun peek tx xs =
  case tx xs of SOME (OK y, _) => SOME y
              | _ => NONE
(* type declarations for consistency checking *)
val _ = op peek : ('a, 'b) xformer -> 'a stream -> 'b option
(* stream transformers and their combinators 1168b *)
fun rewind tx xs =
  case tx xs of SOME (ey, _) => SOME (ey, xs)
              | NONE => NONE
(* type declarations for consistency checking *)
val _ = op rewind : ('a, 'b) xformer -> ('a, 'b) xformer
(* stream transformers and their combinators 1168c *)
fun sat p tx xs =
  case tx xs
...
(* stream transformers and their combinators 1168d *)
fun eqx y = 
  sat (fn y' => y = y') 
(* type declarations for consistency checking *)
...
(* type declarations for consistency checking *)
val _ = op <&> : ('a, 'b) xformer * ('a, 'c) xformer -> ('a, 'c) xformer
(* stream transformers and their combinators 1169c *)
...
(* stream transformers and their combinators 1169d *)
fun many t = 
...
val _ = op <$>! : ('b -> 'c error)             * ('a, 'b) xformer -> ('a, 'c)
                                                                         xformer
(* support for source-code locations and located streams 1156c *)
type srcloc = string * int
fun srclocString (source, line) =
  source ^ ", line " ^ intString line
(* support for source-code locations and located streams 1156d *)
datatype error_format = WITH_LOCATIONS | WITHOUT_LOCATIONS
val toplevel_error_format = ref WITH_LOCATIONS
(* support for source-code locations and located streams 1157a *)
...
(* support for source-code locations and located streams 1157b *)
exception Located of srcloc * exn
(* support for source-code locations and located streams 1157c *)
fun atLoc loc f a =
  f a handle e as RuntimeError _ => raise Located (loc, e)
           | e as NotFound _     => raise Located (loc, e)
           (* more handlers for [[atLoc]] 1157d *)
           ...
           (* more handlers for [[atLoc]] ((type-checking)) 400d *)
...
(* support for source-code locations and located streams 1158a *)
...
(* support for source-code locations and located streams 1158b *)
fun errorAt msg loc = 
  ERROR (synerrormsg loc [msg])
(* support for source-code locations and located streams 1158c *)
type 'a located = srcloc * 'a
(* type declarations for consistency checking *)
val _ = op errorAt : string -> srcloc -> 'a error
(* type declarations for consistency checking *)
type 'a located = 'a located
(* support for source-code locations and located streams 1158d *)
...
(* streams that track line boundaries 1175a *)
...
(* streams that track line boundaries 1175b *)
local 
...
(* support for lexical analysis 1171b *)
type 'a lexer = (char, 'a) xformer
(* type declarations for consistency checking *)
type 'a lexer = 'a lexer
(* support for lexical analysis 1171c *)
fun isDelim c =
  Char.isSpace c orelse Char.contains "()[]{};" c
(* type declarations for consistency checking *)
val _ = op isDelim : char -> bool
(* support for lexical analysis 1173a *)
val whitespace = many (sat Char.isSpace one)
(* type declarations for consistency checking *)
val _ = op whitespace : char list lexer
(* support for lexical analysis 1173b *)
...
(* support for lexical analysis 1173c *)
fun intFromChars (#"-" :: cs) = 
      intFromChars cs >>=+ Int.~
  | intFromChars cs =
...
(* support for lexical analysis 1173d *)
fun intToken isDelim =
  intFromChars <$>! intChars isDelim
(* type declarations for consistency checking *)
val _ = op intToken : (char -> bool) -> int lexer
(* support for lexical analysis 1174a *)
...
fun rightString ROUND  = ")"
  | rightString SQUARE = "]"
  | rightString CURLY = "}"
(* support for lexical analysis 1174b *)
...
(* common parsing code 1162 *)
(* combinators and utilities for parsing located streams 1175c *)
type ('t, 'a) polyparser = ('t located eol_marked, 'a) xformer
(* combinators and utilities for parsing located streams 1176a *)
...
(* combinators and utilities for parsing located streams 1176b *)
fun @@ p = pair <$> srcloc <*> p
(* type declarations for consistency checking *)
val _ = op @@ : ('t, 'a) polyparser -> ('t, 'a located) polyparser
(* combinators and utilities for parsing located streams 1176c *)
infix 0 <?>
fun p <?> what = p <|> errorAt ("expected " ^ what) <$>! srcloc
(* type declarations for consistency checking *)
val _ = op <?> : ('t, 'a) polyparser * string -> ('t, 'a) polyparser
(* combinators and utilities for parsing located streams 1177 *)
infix 4 <!>
fun p <!> msg =
...
(* combinators and utilities for parsing located streams 1180d *)
...
(* transformers for interchangeable brackets 1178 *)
...
(* transformers for interchangeable brackets 1179 *)
...
val _ = op matchBrackets : string -> bracket_shape located -> 'a -> right_result
                                                                     -> 'a error
(* transformers for interchangeable brackets 1180a *)
...
(* transformers for interchangeable brackets 1180b *)
...
val _ = op usageParser : (string -> ('t, string) pb_parser) -> string * ('t, 'a)
                                                 pb_parser -> ('t, 'a) pb_parser
(* transformers for interchangeable brackets 1180c *)
fun pretoken stream = ((fn PRETOKEN t => SOME t | _ => NONE) <$>? token) stream
(* code used to debug parsers 1181a *)
...
(* code used to debug parsers 1181b *)
...
val _ = op showErrorInput : ('t -> string) -> ('t, 'a) polyparser -> ('t, 'a)
                                                                      polyparser
(* code used to debug parsers 1182a *)
...
val _ = op wrapAround : ('t -> string) -> string -> ('t, 'a) polyparser -> ('t,
                                                                  'a) polyparser
(* streams that issue two forms of prompts 1182b *)
...
(* streams that issue two forms of prompts 1183a *)
...
(* streams that issue two forms of prompts 1183b *)
fun lexLineWith lexer =
  stripAndReportErrors o streamOfUnfold lexer o streamOfList o explode
(* type declarations for consistency checking *)
val _ = op lexLineWith : 't lexer -> line -> 't stream
(* streams that issue two forms of prompts 1183c *)
...
val _ = op parseWithErrors : ('t, 'a) polyparser -> 't located eol_marked stream
                                                              -> 'a error stream
(* streams that issue two forms of prompts 1183d *)
...
(* streams that issue two forms of prompts 1184 *)
...
(* shared utility functions for initializing interpreters 371a *)
...
fun setup_error_format interactivity =
  if prompts interactivity then
    ...
    before override_if_testing () (*OMIT*)
(* function [[forward]], for mutual recursion through mutable reference cells 1144a *)
fun forward what _ =
...
(* abstract syntax and values for {\tuscheme} 421d *)
(* definitions of [[exp]] and [[value]] for {\tuscheme} 420c *)
...
(* definitions of [[exp]] and [[value]] for {\tuscheme} 421a *)
and    value = NIL
             | BOOLV     of bool   
             | NUM       of int
             | SYM       of name
...
(* definition of [[expString]] for {\tuscheme} 1282b *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
fun tysubst (tau, varenv) =
  let
      ...
      fun renameForallAvoiding (alphas : name list, tau : tyex, captured : name set) =
            let fun ren (a::[]) = if (List.exists 
                        (fn x => case String.compare(x,a) of EQUAL => true
                                                       | _ => false) 
                    captured) then 
                    freshName(a,captured)::[] else a::[]
                | ren (a::ass) = if (List.exists 
                        (fn x => case String.compare(x,a) of EQUAL => true
                                                       | _ => false) 
                    captured) then
                    (freshName(a,captured)::(ren ass)) else (a::(ren ass))
                | ren _ = raise RuntimeError "wrong"
            in FORALL (ren alphas,tau)
            end
...
(* type checking for {\tuscheme} ((prototype)) 426 *)
fun typeof (e : exp, Delta : kind env, Gamma : tyex env) : tyex =
  let fun ty (LITERAL (NUM n)) = inttype
        ...
        | ty (LITERAL NIL) = FORALL ("'a"::[],TYCON "(list 'a)")
        | ty (LITERAL (PAIR (v, NIL))) = TYCON("(list " ^ typeString(ty (LITERAL v)) ^ ")") 
        | ty (LITERAL (PAIR (h, t))) = (case t of
            (PAIR (x, y)) => if eqType(ty (LITERAL h),ty (LITERAL x)) then
            TYCON("(list " ^ typeString(ty (LITERAL h)) ^ ")") else
            raise TypeError ("mixed types " ^ typeString(ty (LITERAL h)) ^ " and " ^ typeString(ty (LITERAL t)) ^ "in list literal")
            | _ => raise TypeError "not pair")
        ...
        | ty (VAR v) = (find(v,Gamma) handle NotFound msg => raise TypeError ("unknown variable " ^ v))
        | ty (SET (x, e)) =
            let val tau = find(x,Gamma) handle NotFound msg => raise TypeError ("unknown variable " ^ x)
            in if eqType(tau,typeof(e,Delta,Gamma)) then tau else raise TypeError "Tried to set wrong type"
            end
        | ty (IFX (e1, e2, e3)) =
            if eqType(booltype,typeof(e1,Delta,Gamma)) then
                if (eqType(typeof(e2,Delta,Gamma),typeof(e3,Delta,Gamma))) then
                typeof(e2,Delta,Gamma) else 
                raise TypeError "return expressions do not match" else
                raise TypeError ("Condition in if expression has type " ^ typeString(typeof(e1,Delta,Gamma)) ^ ", which should be bool")
        | ty (WHILEX (e1, e2)) = 
            if eqType(booltype,typeof(e1,Delta,Gamma)) then
                typeof(e2,Delta,Gamma) else 
            raise TypeError ("Condition in while expression has type " ^ typeString(typeof(e1,Delta,Gamma)) ^ ", which should be bool")
        | ty (BEGIN es) = (case es of 
             (e::[]) => typeof(e,Delta,Gamma)
            | (e::es) => typeof(BEGIN es,Delta,bind(typeString(typeof(e,Delta,Gamma)),typeof(e,Delta,Gamma),Gamma))
            | [] => unittype) 
        | ty (LETX (LET, bs, body)) = 
            let val env =
            foldr (fn ((n,e),env) => bind(n,typeof(e,Delta,Gamma),env)) [] bs
            in typeof(body,Delta,env@Gamma)
            end 
        | ty (LETX (LETSTAR, [], body)) = ty body
        | ty (LETX (LETSTAR, (b :: bs), body)) = ty (LETX (LET, [b], LETX (LETSTAR, bs, body)))
        | ty (LAMBDA (formals, body)) =
            let val env =
            foldr (fn ((n,e),env) => bind(n,e,env)) [] formals
            val taus = foldr (fn ((n,e),env) => asType(e,Delta)::env) [] formals
            in FUNTY(taus,typeof(body,Delta,env@Gamma))
            end 
        | ty (APPLY (f, actuals)) =
            (let val env =
            foldr (fn (a,env) => bind(typeString(typeof(a,Delta,env)),typeof(a,Delta,env),env)) Gamma actuals 
            val acttypes = map ty actuals
            in (case typeof(f,Delta,Gamma) of
                (FUNTY(taus,result)) => if eqTypes(taus,acttypes) then result else 
                raise TypeError "function recieved incorrect arguments"
                | _  => raise TypeError ("non-function of type " ^ typeString(typeof(f,Delta,Gamma))))
            end) 
        | ty (TYLAMBDA (alphas, e)) = raise LeftAsExercise "couldn't figure out TYLAMBDA" 
            (*let val env = foldr (fn (x,env) => bind(x,TYVAR(x),env)) [] alphas
            val newk = foldr (fn (x,env) => (x,kindof(TYVAR(x),env))::env) Delta alphas
            in FORALL(alphas,typeof(e,newk,env)) handle TypeError g => raise TypeError ("in forall "^g^" alphas " ^ List.hd(alphas))
            end*)
  ...
  case d
    of VAL (name, e) => 
        (case typeof (e,Delta,Gamma) of
            TYCON tyname => (bind(name,typeof(e,Delta,Gamma),Gamma),tyname)
            | CONAPP (t1,ts) => (Gamma,typeString(t1))
            | FUNTY (tylist) => (bind(name,typeof(e,Delta,Gamma),Gamma),typeString(FUNTY tylist)) 
            | FORALL (ns,t1) => (foldr 
                (fn (n,oldv) => bind(n,t1,oldv))
            Gamma ns,typeString(typeof(e,Delta,Gamma)))
            | TYVAR tyname => (bind(name,typeof(e,Delta,Gamma),Gamma),tyname))
     | EXP e => elabdef (VAL ("it", e), Delta, Gamma)
     | DEFINE (name, tau, lambda as (formals, body)) =>
        let val e = bind(name,tau,Gamma)
            val en = foldr (fn ((str,t),e) => bind(str,t,e)) e formals
        in if not(appearsUnprotectedIn(name,body)) then
            let val env= bind(name,typeof(LAMBDA(formals,body),Delta,en),en)
            in (env,typeString(typeof(LAMBDA(formals,body),Delta,en)))
            end else
            let val taus = foldr (fn ((n,e),env) => asType(e,Delta)::env) [] formals
                val env = bind(name,FUNTY(taus,tau),en)
                val na= typeString(typeof(LAMBDA(formals,body),Delta,env))
            in (bind(name,typeof(LAMBDA(formals,body),Delta,env),env),na)
            end 
        end
     | VALREC (name, tau, e) => 
        let val env = bind(name,tau,Gamma)
        in case e of
            (LAMBDA x) => 
            if eqType(tau,typeof(e,Delta,env)) then 
            (bind(name,tau,env),typeString(typeof(e,Delta,env))) else 
            raise TypeError "val-rec returns incorrect type"
            | _ => 
            if eqType(tau,typeof(e,Delta,Gamma)) then 
            (bind(name,tau,Gamma),typeString(typeof(e,Delta,Gamma))) else 
            raise TypeError "val-rec returns incorrect type"
        end 
(* type declarations for consistency checking *)
...
(* lexical analysis and parsing for \tuscheme, providing [[filexdefs]] and [[stringsxdefs]] 1273b *)
(* lexical analysis for \uscheme\ and related languages 1253b *)
datatype pretoken = QUOTE
                  | INT     of int
                  | SHARP   of bool
                  | NAME    of string
type token = pretoken plus_brackets
(* lexical analysis for \uscheme\ and related languages 1254a *)
...
(* lexical analysis for \uscheme\ and related languages 1254b *)
local
  (* functions used in all lexers 1254d *)
  ...
  (* functions used in the lexer for \uscheme 1254c *)
...
(* parsers for single \uscheme\ tokens 1255a *)
...
val _ = op distinctBsIn : (name * 'e) list parser -> string -> (name * 'e) list
                                                                          parser
(* parsers and parser builders for formal parameters and bindings 1255c *)
...
(* parsers and parser builders for formal parameters and bindings 1256a *)
fun kw keyword = 
...
(* parsers and parser builders for \scheme-like syntax 1256b *)
fun sexp tokens = (
     SYM       <$> (notDot <$>! @@ any_name)
 <|> NUM       <$> int
 <|> embedBool <$> booltok
...
(* parsers and parser builders for \scheme-like syntax 1256c *)
fun atomicSchemeExpOf name =  VAR                   <$> name
                          <|> LITERAL <$> NUM       <$> int
                          <|> LITERAL <$> embedBool <$> booltok
(* parsers and parser builders for \scheme-like syntax 1257c *)
...
(* parser builders for typed languages 1267a *)
...
(* parser builders for typed languages 1274c *)
val distinctTyvars = 
...
(* parsers and [[xdef]] streams for {\tuscheme} 1274a *)
fun keyword words =
...
(* parsers and [[xdef]] streams for {\tuscheme} 1274d *)
...
(* parsers and [[xdef]] streams for {\tuscheme} 1275a *)
...
(* parsers and [[xdef]] streams for {\tuscheme} 1275b *)
val unit_test =
...
(* parsers and [[xdef]] streams for {\tuscheme} 1276a *)
...
val xdef = 
 ...
 <|> DEF <$> EXP <$> exp
 <?> "definition"
(* parsers and [[xdef]] streams for {\tuscheme} 1276b *)
val xdefstream = interactiveParsedStream (schemeToken, xdef)
(* shared definitions of [[filexdefs]] and [[stringsxdefs]] 1156b *)
...
(* evaluation, testing, and the read-eval-print loop for {\tuscheme} 1276c *)
(* definition of [[namedValueString]] for functional bridge languages 1278b *)
...
(* definitions of [[eval]] and [[evaldef]] for {\tuscheme} 1276d *)
fun eval (e, rho) =
  let fun ev (LITERAL n) = n
        (* alternatives for [[ev]] for [[TYAPPLY]] and [[TYLAMBDA]] 441 *)
        | ev (TYAPPLY  (e, _)) = ev e
        | ev (TYLAMBDA (_, e)) = ev e
        (* more alternatives for [[ev]] for {\tuscheme} 1277a *)
        | ev (VAR v) = !(find (v, rho))
        | ev (SET (n, e)) = 
            let val v = ev e
            in  find (n, rho) := v;
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1277b *)
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1277c *)
        | ev (LAMBDA (args, body)) = CLOSURE ((map (fn (x, ty) => x) args, body)
                                                                          , rho)
        (* more alternatives for [[ev]] for {\tuscheme} 1277d *)
        | ev (APPLY (f, args))  = 
               (case ev f
                  of PRIMITIVE prim => prim (map ev args)
                   | CLOSURE clo =>
                       (* apply closure [[clo]] to [[args]] ((mlscheme)) 360b *)
        ...
        (* more alternatives for [[ev]] for {\tuscheme} 1277e *)
        | ev (LETX (LET, bs, body)) =
...
(* definitions of [[eval]] and [[evaldef]] for {\tuscheme} 1278a *)
...
(* shared unit-testing utilities 1148c *)
fun failtest strings = (app eprint strings; eprint "\n"; false)
(* shared unit-testing utilities 1148d *)
...
(* definition of [[testIsGood]] for {\tuscheme} 1281d *)
fun testIsGood (test, (Delta, Gamma, rho)) =
  let fun ty e = typeof (e, Delta, Gamma)
                 handle NotFound x => raise TypeError ("name " ^ x ^
                                                              " is not defined")

(* shared [[checkExpectChecks]], [[checkErrorChecks]], and [[checkTypeChecks]], which call [[ty]] 1264d *)
...
(* shared [[checkExpectChecks]], [[checkErrorChecks]], and [[checkTypeChecks]], which call [[ty]] 1264e *)
...
(* shared [[checkExpectChecks]], [[checkErrorChecks]], and [[checkTypeChecks]], which call [[ty]] 1282a *)
...
(* shared [[checkExpectPasses]] and [[checkErrorPasses]], which call [[outcome]] 1148b *)

(* shared [[checkExpectPassesWith]] and [[checkErrorPasses]], which call [[outcome]] 1146b *)
...
(* shared [[checkExpectPassesWith]] and [[checkErrorPasses]], which call [[outcome]] 1147 *)
...
(* shared [[checkExpectPassesWith]] and [[checkErrorPasses]], which call [[outcome]] 1148a *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] 1264b *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] 1264c *)
...
(* shared definition of [[processTests]] 1148e *)
...
(* shared read-eval-print loop and [[processPredefined]] 367a *)
fun processPredefined (def, basis) = processDef (def, basis, noninteractive)
(* type declarations for consistency checking *)
val _ = op noninteractive : interactivity
val _ = op processPredefined : def * basis -> basis
(* shared read-eval-print loop and [[processPredefined]] 367c *)
fun readEvalPrintWith errmsg (xdefs, basis, interactivity) =
  let val unitTests = ref []

(* definition of [[processXDef]], which can modify [[unitTests]] and call [[errmsg]] 368b *)
      fun processXDef (xd, basis) =
        let (* definition of [[useFile]], to read from a file 368a *)
     ...
     (* definition of [[complaintOfExn]], why we don't like an exception 369b *)
            fun complaintOfExn exn =
              let fun template exn =
                    case exn
                      of IO.Io { name, ...} => "I/O error <at loc>:" ^ name

                     (* more cases for [[complaintOfExn]]'s [[template]] 369c *)
   ...
   (* more cases for [[complaintOfExn]]'s [[template]] ((type-checking)) 400c *)
                       | TypeError         msg => "type error <at loc>: " ^ msg
                       | BugInTypeChecking msg => "bug in type checking: " ^ msg
                       | LeftAsExercise msg => "left as exercise: " ^ msg
                       | Located _          =>
 ...
 (* handlers that catch non-fatal exceptions and pass them to [[caught]] 369a *)
              | e as Div            => caught e
...
(* handlers that catch non-fatal exceptions and pass them to [[caught]] ((type-checking)) 400b *)
              | e as TypeError _         => caught e
              | e as BugInTypeChecking _ => caught e

              | e as LeftAsExercise _ => caught e
        end 
      val basis = streamFold processXDef basis xdefs
      val _     = processTests (!unitTests, basis)
(* type declarations for consistency checking *)
val _ = op readEvalPrintWith : (string -> unit) -> xdef stream * basis *
                                                          interactivity -> basis
...
(* shared utility functions for building primitives in languages with type checking 1270c *)
...
(* shared utility functions for building primitives in languages with type checking 1270d *)
fun arithOp f =
...
(* utility functions and types for making \tuscheme\ primitives 1278c *)
val arithtype = FUNTY ([inttype, inttype], inttype)
(* type declarations for consistency checking *)
val _ = op arithtype : tyex
(* utility functions and types for making \tuscheme\ primitives 1279a *)
...
val initialBasis =
  let fun addKind ((name, kind), kinds) = bind (name, kind, kinds)
      val kinds   = foldl addKind emptyEnv
                    (
                    (* primitive type constructors for \tuscheme\ [[::]] 415c *)
                     ("int",  TYPE) ::
                     ("bool", TYPE) ::
                     ("sym",  TYPE) ::
                     ("unit", TYPE) ::
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \tuscheme\ [[::]] 1278d *)
                             ("+", arithOp op +,   arithtype) :: 
                             ("-", arithOp op -,   arithtype) :: 
                             ("*", arithOp op *,   arithtype) :: 
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \tuscheme\ [[::]] 1279b *)
                           ...
                           (* primitive functions for \tuscheme\ [[::]] 1279c *)
                           ...
                           (* primitive functions for \tuscheme\ [[::]] 1279d *)
      ...
      val (types, values) =
                      ...
                      (
             (* primitives that aren't functions, for \tuscheme\ [[::]] 1279e *)
...
(* function [[runAs]], which evaluates standard input given [[initialBasis]] 371c *)
...
(* code that looks at command-line arguments and calls [[runAs]] to run the interpreter 371d *)
val _ = case CommandLine.arguments ()
          of []      => runAs (PROMPTING,     PRINTING)
           | ["-q"]  => runAs (NOT_PROMPTING, PRINTING)
           | ["-qq"] => runAs (NOT_PROMPTING, NOT_PRINTING)
