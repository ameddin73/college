...
(* types for \timpcore 387a *)
(* definitions of [[typeString]] and [[funtyString]] for \timpcore 1265b *)
fun typeString BOOLTY        = "bool"
  | typeString INTTY         = "int"
  | typeString UNITTY        = "unit"
  | typeString (ARRAYTY tau) = "(array " ^ typeString tau ^ ")"
(* definitions of [[typeString]] and [[funtyString]] for \timpcore 1265c *)
...
(* abstract syntax and values for \timpcore 388d *)
(* definitions of [[exp]] and [[value]] for \timpcore 387d *)
datatype value = NUM   of int
               | ARRAY of value array
(* definitions of [[exp]] and [[value]] for \timpcore 387e *)
datatype exp   = LITERAL of value
               ...
               (* array extensions to \timpcore's abstract syntax 404a *)
               | AAT   of exp * exp
               | APUT  of exp * exp * exp
               | AMAKE of exp * exp
               | ASIZE of exp
(* definition of type [[func]], to represent a \timpcore\ function 388c *)
datatype func = USERDEF   of name list * exp
              | PRIMITIVE of value list -> value
(* definition of [[def]] for \timpcore 388a *)
type userfun = { formals : (name * ty) list, body : exp, returns : ty }
datatype def = VAL    of name * exp
             | EXP    of exp
             | DEFINE of name * userfun
(* definition of [[unit_test]] for \timpcore 388b *)
...
(* definition of [[valueString]] for \timpcore 1265d *)
...
(* definition of [[expString]] for \timpcore 1265a *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* abstract syntax and values for \timpcore 404b *)
...
(* utility functions on \timpcore\ values 1263a *)
...
fun typeof (e, globals, functions, formals) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 395a *)
        | ty (VAR x) = (find (x, formals)
                        handle NotFound _ =>
                           (find (x, globals)
                            handle NotFound _ =>
                               raise TypeError ("unknown variable " ^ x)))
        ...
        | ty (APPLY (f, actuals)) =
             let val actualtypes                     = map ty actuals
                 val FUNTY (formaltypes, resulttype) = (find (f, functions)
                                                        handle NotFound _ =>
                                                           raise TypeError ("unknown function " ^ f))
                 ...
                 fun parameterError (n, atau::actuals, ftau::formals) =
                   ...
                   | parameterError _ =
                       raise TypeError ("Function " ^ f ^ " expects " ^
                                        intString (length formaltypes) ^ " parameters " ^
...
(* function [[ty]], to check the type of an expression, given $\itenvs$ ((prototype)) 405 *)
        | ty (AMAKE (len, init)) = (case typeof(len,globals,functions,formals) of
            INTTY => ARRAYTY(typeof(init,globals,functions,formals))
            | _ => raise TypeError "Length is not of type int")
        | ty (AAT (a, i))        = (case (typeof(a,globals,functions,formals),typeof(i,globals,functions,formals)) of
            (ARRAYTY t1, INTTY) => t1
            | _ => raise TypeError "Type in array at is not correct")
        | ty (APUT (a, i, e))    = (case (typeof(a,globals,functions,formals),typeof(i,globals,functions,formals),typeof(e,globals,functions,formals)) of
            (ARRAYTY t1,INTTY,t2) => if eqType(t1,t2) then t1 else raise TypeError "not right type"
            | _ => raise TypeError "not array")
        | ty (ASIZE a)           = case typeof(a,globals,functions,formals) of 
            (ARRAYTY t1) => INTTY 
            | _ => raise TypeError "Array in array-size expression is not array type"
(* type declarations for consistency checking *)
...
(* type checking for \timpcore 398b *)
fun elabdef (d, globals, functions) =
    case d
      of (* cases for elaborating definitions in \timpcore 398c *)
           VAL (x, e) =>
             let val x's_type = SOME (find (x, globals)) handle NotFound _ => NONE
                    ...
                    | SOME tau' =>  (* OldVal *)
                        if eqType (tau, tau') then
                          ...
                          raise TypeError ("Global variable " ^ x ^ " of type " ^
                                           typeString (find (x, globals)) ^ " may not be " ^
                                           "redefined with type " ^ typeString tau)
             end
         (* cases for elaborating definitions in \timpcore 399a *)
         | EXP e =>
             let val tau = typeof (e, globals, functions, emptyEnv)
             in  (globals, functions, typeString tau)
             end
         (* cases for elaborating definitions in \timpcore 399b *)
         | DEFINE (f, {returns, formals, body}) =>
             let val f's_type = SOME (find (f, functions)) handle NotFound _ => NONE
         ...
         val _ = op f's_type : funty option
             in  if eqType (tau, returns) then
                   (case f's_type
                      of NONE => (globals, functions', funtyString definition's_type)
                       | SOME funty =>
                           if eqFunty (funty, definition's_type) then
                             (globals, functions, funtyString definition's_type)
                           else
                             raise TypeError ("Function " ^ f ^ " of type " ^ funtyString funty ^
...
(* lexical analysis and parsing for \timpcore, providing [[filexdefs]] and [[stringsxdefs]] 1266a *)
...
(* parsers and [[xdef]] streams for \timpcore 1266b *)
...
(* parsers and [[xdef]] streams for \timpcore 1267b *)
...
(* parsers and [[xdef]] streams for \timpcore 1267c *)
...
(* parsers and [[xdef]] streams for \timpcore 1267d *)
...
val xdeftable = usageParsers
  ...
  (* rows added to \timpcore\ [[xdeftable]] in exercises 1268b *)
...
val xdef =  DEF  <$> deftable 
        ...
        <|> DEF <$> EXP <$> exp
        <?> "definition"
(* parsers and [[xdef]] streams for \timpcore 1268a *)
...
(* evaluation, testing, and the read-eval-print loop for \timpcore 1268c *)
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1268d *)
val unitVal = NUM 1983
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1269a *)
fun eval (e, globals, functions, formals) =
  let fun toBool (NUM 0) = false
      ...
      fun ev (LITERAL n)          = n
        ...
        | ev (APPLY (f, args)) =
            (case find (f, functions)
               of PRIMITIVE p  => p (map ev args)
                | USERDEF func =>
                    (* apply user-defined function [[func]] to [[args]] 1269b *)
        ...
        (* more alternatives for [[ev]] for \timpcore 404c *)
        | ev (AAT (a, i)) =
...
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1270a *)
fun evaldef (d, globals, functions) =
  case d 
    of VAL (x, e) => (* evaluate [[e]] and bind the result to [[x]] 1270b *)
...
val _ = op evaldef : def * value ref env * func env -> value ref env * func env
                                                                        * string
(* definitions of [[basis]] and [[processDef]] for \timpcore 400a *)
...
(* definition of [[testIsGood]] for \timpcore 1263b *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] 1264c *)
      ...
      (* definition of [[checkFunctionTypePasses]] 1264a *)
...
(* implementations of \timpcore\ primitives and definition of [[initialBasis]] 401a *)
...
(* utility functions and types for making \timpcore\ primitives 1270e *)
val arithtype = FUNTY ([INTTY, INTTY], INTTY)
(* type declarations for consistency checking *)
val _ = op arithtype : funty
(* utility functions and types for making \timpcore\ primitives 1271a *)
...
val initialBasis =
  let fun addPrim ((name, prim, funty), (tfuns, vfuns)) = 
      ...
      val (tfuns, vfuns)  = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \timpcore\ [[::]] 1270f *)
                             ("+", arithOp op +,   arithtype) :: 
                             ("-", arithOp op -,   arithtype) :: 
                             ("*", arithOp op *,   arithtype) :: 
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \timpcore\ [[::]] 1270g *)
                           ...
                           (* primitive functions for \timpcore\ [[::]] 1271b *)
