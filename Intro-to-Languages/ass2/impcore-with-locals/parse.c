...
Exp reduce_to_exp(int code, struct Component *components) {
    ...
    /* cases for \impcore's [[reduce_to_exp]] added in exercises 1101d */
    /* add your syntactic extensions here */
    /* cases for \impcore's [[reduce_to_exp]] added in exercises 1116b */
...
/* arrays of shift functions added to \impcore\ in exercises 1110c */
/* for each new row added to exptable, add an array of shift functions here */
/* arrays of shift functions added to \impcore\ in exercises 1117a */
...
struct ParserRow exptable[] = {
  ...
  /* rows added to \impcore's [[exptable]] in exercises 1110d */
  /* add a row here for each new syntactic form of Exp */
  /* rows added to \impcore's [[exptable]] in exercises 1117b */
...
struct Usage usage_table[] = {
    { ADEF(VAL),           "(val x e)" },
    { ADEF(DEFINE),        "(define fun (formals) [(locals locals)] body)" },
    ...
    { BEGIN,  "(begin exp ... exp)" },
    /* \impcore\ [[usage_table]] entries added in exercises 1117c */
    { SUGAR(CAND), "(&& exp exp)" },
    { SUGAR(COR),  "(|| exp exp)" },
